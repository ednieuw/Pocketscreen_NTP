...
void ConnectToWiFiNetwork(void)
{
  ...
  espATCommand("AT+CWJAP=\"FRITZ!BoxEd\",\"password\"", "OK", LONG_PAUSE);    // connect to wifi
  espATCommand("AT+CIPMUX=1", OK_STR, SHORT_PAUSE);                            //set the single connection mode
}
...
Tekstprintln("In Epoch1"); 
  ...
  //AT+CIPSTART="UDP","130.102.128.23",123
// AT+CIPSTART="UDP","nl.pool.ntp.org",123
// ntp0.nl.net   193.78.240.12
// AT+CIPSTART="UDP","130.102.128.23",123
// https://www.arduino.cc/en/Tutorial/MKRNBExamplesGPRSUdpNtpClient
// https://github.com/PaulStoffregen/Time/blob/master/examples/TimeNTP_ESP8266WiFi/TimeNTP_ESP8266WiFi.ino
  ...
  SerialESP.write(packetBuffer, NTP_PACKET_SIZE);                               // the first 4 bytes matters, then we don't care, whatever is in the memory will do
  Tekstprint("NTP packet send:"); 
  SerialUSB.write(packetBuffer, NTP_PACKET_SIZE); 
...
//  Tekstprintln("NTP packet"); 
//  SerialUSB.write((char*) &ntpFirstFourBytes, NTP_PACKET_SIZE); 
...
Tekstprintln("In Epoch3"); 
  ...
  espATCommand("AT+CIPCLOSE=4", OK_STR, SHORT_PAUSE); // close connection
